...
const {
  ADDRESSES: {
    ...
    POLYGON: { PNT: PNT_ON_POLYGON, PNT_MINTER: PNT_MINTER_ON_POLYGON }
  },
  MISC: { ONE_DAY },
...
const WITHDRAW_INFLATION_FROM_GNOSIS_USER_DATA =
  // secretlint-disable-next-line
  ''
...
describe('Integration tests on Gnosis deployment', () => {
  ...
  const missingSteps = async () => {
    const Forwader = await ethers.getContractFactory('Forwarder')
    forwarder = await Forwader.deploy(PNT_ON_GNOSIS, ethers.ZeroAddress)
    await forwarder.whitelistOriginAddress(PNETWORK_NETWORK_IDS.POLYGON, ADDRESS_PLACEHOLDER.toLowerCase())
    await forwarder.addUnprivilegedCall('0x095ea7b3')
  ...
  it('[dapp] should stake from forwarder call', async () => {
    ...
    const metadata = encodeMetadata(ethers, {
      ...
      senderAddress: ADDRESS_PLACEHOLDER,
...
describe('Integration tests on Ethereum deployment', () => {
  ...
  const missingSteps = async () => {
    ...
    crossExecutor = await CrossExecutor.deploy(PNT_ON_ETH, ERC20_VAULT, DANDELION_VOTING)
    await crossExecutor.whitelistOriginAddress(PNETWORK_NETWORK_IDS.GNOSIS, ADDRESS_PLACEHOLDER.toLocaleLowerCase())
  ...
  it.skip('[dapp] should open a vote for migrating ethPNT treasury funds (3)', async () => {
    await crossExecutor
      .connect(safe)
      .call(
        ethPnt.target,
  ...
  it.skip('should be able to change inflationOwner from CrossExecutor', async () => {
    await expect(
      crossExecutor
        .connect(safe)
  ...
  it.skip('should not be able to change inflationOwner from CrossExecutor without ownership', async () => {
    await expect(
      crossExecutor
        .connect(association)
        .call(ETHPNT, ethPnt.interface.encodeFunctionData('setInflationOwner', [association.address]))
...
describe('Integration tests on Polygon deployment', () => {
  ...
  const missingSteps = async () => {
    const Forwader = await ethers.getContractFactory('Forwarder')
    forwarder = await Forwader.deploy(PNT_ON_POLYGON, ethers.ZeroAddress)
  ...
  it('[dapp] should call forwarder for staking', async () => {
    ...
    await expect(
      ...
      .withArgs(
        forwarder.target,
        100000000000000000n,
        ADDRESS_PLACEHOLDER.toLowerCase().slice(2),
        // secretlint-disable-next-line
